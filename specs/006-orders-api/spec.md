...
### Session 2025-12-08
- Q: What filter parameters should the Orders list endpoint support beyond pagination? → A: Support companyId and status filters (matches typical Pax8 API pattern).
- Q: What order status values should the API client support for filtering? → A: Comprehensive set: Pending, Processing, Completed, Cancelled, PartiallyCompleted, Error.
- Q: Should the Order entity include additional line item details beyond productId and quantity? → A: Extended: Add price, provisioningDetails, and billingTerm.
- Q: Should order creation support specifying a billing term per line item? → A: Required when product has multiple terms.
...
Partner/integrator retrieves a paginated list of orders to view purchasing history, track order status, or sync order data into their internal systems.
...
**Independent Test**: Call the orders list endpoint with optional filters (page, size, sort); verify the response includes order records with pagination metadata and is usable standalone for order history browsing.
...
3. **Given** valid authorization and a sort parameter (e.g., "createdDate,desc"), **When** the client requests the orders list, **Then** orders are returned in the specified sort order.
...
Partner/integrator retrieves full details of a specific order to view line items, quantities, status, and associated company information for order tracking or support purposes.
...
1. **Given** a valid order identifier and authorization, **When** the client requests the order detail, **Then** the response returns the order with all attributes including id, companyId, lineItems, status, and timestamps.
...
**Independent Test**: Submit a valid order request with companyId and lineItems; verify the response returns the created order with an assigned ID and initial status.

**Acceptance Scenarios**:

1. **Given** valid authorization, a valid companyId, and valid lineItems (productId and quantity), **When** the client creates an order, **Then** the response returns the created order with an assigned id, the specified companyId, lineItems, and an initial order status.
...
- **FR-002**: System MUST support filtering the orders list by `companyId` (to scope orders to a specific company) and `status` (to filter by order processing state).
- **FR-003**: System MUST support sorting the orders list by createdDate with optional direction (ascending or descending).
- **FR-004**: System MUST provide an order detail endpoint that returns a single order by ID with all attributes including id, companyId, lineItems (with productId, quantity, price, billingTerm, subscriptionId), status, and timestamps.
- **FR-005**: System MUST provide an order creation endpoint that accepts companyId and an array of lineItems (each with productId, quantity, billingTerm, and optional provisioningDetails).
- **FR-006**: System MUST validate order creation requests: companyId must exist and be accessible, all productIds must be valid, quantities must be positive integers, billingTerm must be valid for the product.
...
- **FR-008**: System MUST include pagination metadata in list responses: page number, page size, total elements, and total pages.
- **FR-009**: System MUST enforce request validation limits (e.g., maximum page size) and return validation errors without processing the request.
- **FR-010**: System MUST return the created order with an assigned id and initial status upon successful order creation.

### Key Entities

- **Order**: Represents a purchase record. Attributes: `id` (uuid), `companyId` (reference to company), `lineItems` (array of OrderLineItem), `status` (order processing status: `Pending` | `Processing` | `Completed` | `Cancelled` | `PartiallyCompleted` | `Error`), `createdDate` (ISO 8601 timestamp), `updatedDate` (ISO 8601 timestamp).
- **OrderLineItem**: Represents a single product line in an order. Attributes: `productId` (reference to product), `quantity` (positive integer), `lineItemNumber` (sequence within order), `price` (unit price as number), `billingTerm` (Monthly | Annual | 2-Year | 3-Year | One-Time | Trial | Activation), `provisioningDetails` (array of key-value pairs for product configuration), `subscriptionId` (nullable, reference to resulting subscription once created).
- **CreateOrderRequest**: Request payload for order creation. Attributes: `companyId` (required, string), `lineItems` (required, array of CreateOrderLineItem).
- **CreateOrderLineItem**: Line item in creation request. Attributes: `productId` (required, string), `quantity` (required, positive integer), `billingTerm` (required when product has multiple terms: Monthly | Annual | 2-Year | 3-Year | One-Time | Trial | Activation), `provisioningDetails` (optional, array of key-value pairs for product configuration).
- **ListOrdersOptions**: Query parameters for listing orders. Attributes: `page` (0-indexed page number), `size` (page size, 1-200), `sort` (e.g., "createdDate,desc"), `companyId` (optional filter by company), `status` (optional filter by order status).
...
- Order status values will follow Pax8 API conventions (e.g., Pending, Processing, Completed, Cancelled).
...
- **SC-005**: 100% of invalid order creation requests (missing companyId, invalid productId, invalid quantity) return appropriate validation errors without creating partial orders.
